...
using Core.Hybrid.Hybrid.PerlinNoise;
...
{
    [CustomEditor(typeof(PerlinNoiseWorldSpaceCombinedBlobSo))]
    class PerlinNoiseWorldSpaceCombinedBlobEditor : UnityEditor.Editor
    {
        PerlinNoiseWorldSpaceCombinedBlobSo Target => target as PerlinNoiseWorldSpaceCombinedBlobSo;
...
#pragma warning disable 0649 // assigned but never used
        private PerlinNoise3dParams tpDef;
        private PerlinNoiseParams npDef;
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (mPosChannels == null)
                mPosChannels = SetupReorderableLists(
                    serializedObject.FindProperty(nameof(Target.positions)), mPoslabels);
            if (mRotChannels == null)
                mRotChannels = SetupReorderableLists(
                    serializedObject.FindProperty(nameof(Target.directions)), mRotlabels);
            ...
            {
                mSampleCachePos.SnapshotSample(r.size, Target.positions, mNoiseOffset, mAnimatedPreview);
            ...
            if (Event.current.type == EventType.Repaint)
            {
                mSampleCacheRot.SnapshotSample(r.size, Target.directions, mNoiseOffset, mAnimatedPreview);
        ...
        class SampleCache
        {
            ...
            public void SnapshotSample(
                Vector2 areaSize, PerlinNoise3dParams[] signal, float noiseOffset, bool animated)
            {
                ...
                for (int i = 0; i < numSamples; ++i)
                {
                    ...
                    Vector3 p = PerlinNoiseWorldSpaceCombinedBlob.GetCombined(signal, t, Vector3.zero);
                    mSampleNoise.Add(p);
                }
            ...
            SerializedProperty property, int channel, GUIContent title)
        {
            ...
            list.drawHeaderCallback = (Rect rect) =>
            {
                ...
                r.x = rect.x + rect.width - steadyLabelWidth;
                r.width = steadyLabelWidth;
            ...
            list.drawElementCallback
                = (Rect rect, int index, bool isActive, bool isFocused) =>
                {
                    ...
                    {
                        case 0:
                            DrawNoiseChannel(rect, element.FindPropertyRelative(nameof(PerlinNoise3dParams.x)));
                            break;
                        case 1:
                            DrawNoiseChannel(rect, element.FindPropertyRelative(nameof(PerlinNoise3dParams.y)));
                            break;
                        case 2:
                            DrawNoiseChannel(rect, element.FindPropertyRelative(nameof(PerlinNoise3dParams.z)));
                            break;
                        default: break;
                    }
                };
            ...
            list.onAddCallback = (ReorderableList l) =>
            {
                ...
                ClearComponent(p.FindPropertyRelative(nameof(PerlinNoise3dParams.x)));
                ClearComponent(p.FindPropertyRelative(nameof(PerlinNoise3dParams.y)));
                ClearComponent(p.FindPropertyRelative(nameof(PerlinNoise3dParams.z)));
            };
            ...
            list.onRemoveCallback = (ReorderableList l) =>
            {
                ...
                bool IsClear
                    = (list.mChannel == 0 || IsClearComponent(p.FindPropertyRelative(nameof(PerlinNoise3dParams.x))))
                      && (list.mChannel == 1 || IsClearComponent(p.FindPropertyRelative(nameof(PerlinNoise3dParams.y))))
                      && (list.mChannel == 2 || IsClearComponent(p.FindPropertyRelative(nameof(PerlinNoise3dParams.z))));
                if (IsClear)
        ...
        private void InitializeLabels(SerializedProperty property)
        {
            if (steadyLabel == null)
            {
                SerializedProperty p = property.FindPropertyRelative(nameof(PerlinNoiseParams.constant));
                steadyLabel = new GUIContent(p.displayName, p.tooltip) { text = " " };
            }

            if (freqLabel == null)
            {
                SerializedProperty p = property.FindPropertyRelative(nameof(PerlinNoiseParams.frequency));
            ...
            if (ampLabel == null)
            {
                SerializedProperty p = property.FindPropertyRelative(nameof(PerlinNoiseParams.amplitude));
        ...
        private void DrawNoiseChannel(Rect rect, SerializedProperty property)
        {
            ...
            EditorGUI.PropertyField(r, property.FindPropertyRelative(nameof(PerlinNoiseParams.frequency)), freqLabel);

            r.x += r.width + hSpace;
            EditorGUIUtility.labelWidth = ampLabelWidth;
            EditorGUI.PropertyField(r, property.FindPropertyRelative(nameof(PerlinNoiseParams.amplitude)), ampLabel);

            r.y -= 1;
            r.x += r.width + hSpace;
            r.width = EditorGUIUtility.singleLineHeight + hSpace;
            EditorGUIUtility.labelWidth = hSpace;
            EditorGUI.PropertyField(r, property.FindPropertyRelative(nameof(PerlinNoiseParams.constant)), steadyLabel);
        ...
        {
            p.FindPropertyRelative(nameof(PerlinNoiseParams.amplitude)).floatValue = 0;
            p.FindPropertyRelative(nameof(PerlinNoiseParams.frequency)).floatValue = 0;
            p.FindPropertyRelative(nameof(PerlinNoiseParams.constant)).boolValue = false;
        }

        // SerializedProperty is a NoiseSettings.NoiseParam
        bool IsClearComponent(SerializedProperty p)
        {
            return p.FindPropertyRelative(nameof(PerlinNoiseParams.amplitude)).floatValue == 0
                   && p.FindPropertyRelative(nameof(PerlinNoiseParams.frequency)).floatValue == 0;
        }
    }
}